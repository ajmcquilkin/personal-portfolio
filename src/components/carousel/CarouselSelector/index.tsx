...
const CarouselSelector = ({
    ...
    const colorClassSelector =
        colorMode === "dark" ? styles.dark : styles.light;

    return (
        <div
            className={[styles.container, colorClassSelector, className].join(
                " ",
            )}
        >
            <button
                ...
                {[...Array(numOptions)].map((_value, idx) => (
                    <button
                        ...
                        className={[
                            styles.dot,
                            colorClassSelector,
                            idx === currentPos ? styles.active : "",
                        ].join(" ")}
                        // eslint-disable-next-line react/no-array-index-key
                        key={idx}
                    >
                        <p>dot selector {idx}</p>
                    </button>
                ))}
            </div>
